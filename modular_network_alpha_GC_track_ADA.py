...
def get_GC_size_individual(G, num_modoules, N):
	...
	mod_size = int(N / num_modules)

	GC_nodes = get_GC_nodes(G)

	final_list_GC = [0 for i in range(num_modoules)]
...
def ADA_attack(G_copy,num_nodes_to_remove,set_of_connected_nodes):
	...
	original_degree_seq = degree_sequence.sort(key = itemgetter(1), reverse = True)
	...
	for i in range(num_nodes_to_remove):
		...
		label = int(node_to_remove / mod_num)
	...
	return (GC_List, SGC_List, num_comp_List, avg_comp_size_List,adaptive_degree_list,connected_removed_nodes,label_list, individual_GC_list)
...
def BA_attack_igraph(G_copy,num_nodes_to_remove, set_of_connected_nodes):
	...
	return (GC_List, SGC_List, num_comp_List, avg_comp_size_List, connected_removed_nodes, label_list)
...
for i in range(num_times):
	...
	(GC_List, SGC_List, num_comp_List, avg_comp_size_List,adaptive_degree_list,connected_removed_nodes,label_list,individual_GC_list) = ADA_attack(G,int(N*0.9),set_of_connected_nodes)
	...
	init_name_individualGC_list = adaptive_type + "SGCattackDEG_" + type_graph + "_individualGCList"
	...
	individualGC_name = get_name_ModularNetworks_alpha(init_name_individualGC_list, N,k_intra,k_inter,alpha,SEED,num_modules,radius)
	...
	with open(individualGC_name,'wb') as handle:
		pickle.dump(individual_GC_list, handle, protocol=pickle.HIGHEST_PROTOCOL)
