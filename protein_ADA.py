...
def make_realworldnetwork(file_name):
	...
	for fn in glob.glob(name_of_data + '*.csv'):
		...
		for idx,line in enumerate(reader):
			...
			if idx == 0:
				for i in range(len(line)):
					name_to_index[line[i]] = i
				continue

			node1.append(int(line[name_to_index["node_1"]]))
			node2.append(int(line[name_to_index["node_2"]]))
	...
	num_nodes = max(max(node1), max(node2))

	G = nx.empty_graph(num_nodes)

	while counter < len(node1):
...
def get_protein_data(name_of_data):
	...
	for fn in glob.glob(name_of_data + '.csv'):
		...
		for new_line in reader:
			...
			if new_line[0] in csv_to_node_dict:
				...
				u = csv_to_node_dict[new_line[0]]

			else:

				u = counter

				csv_to_node_dict[new_line[0]] = counter
			...
			if new_line[1] in csv_to_node_dict:
				...
				csv_to_node_dict[new_line[1]] = counter
