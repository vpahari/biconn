...
def getNextLevel(G,l):
    nextLevel_List = []
    for node in l:
        tempList = list(G.neighbors(node))
        nextLevel_List = nextLevel_List + tempList
    return list(set(nextLevel_List))
...
def takeNodesOut(G, numNodesToRemove):
    ...
    while len(nodesToRemove) < numNodesToRemove:
        nextLevel = getNextLevel(G,nextLevel)

        nextLevel = [x for x in nextLevel if x not in nodesToRemove]

        if len(nextLevel) == 0:
            break

        nodesToRemove = list(set(nodesToRemove + nextLevel))
...
output_file_name = "resultNewUrinalBA_N_%d_k_%d_SEED_%d.csv"%(N,k,SEED)
