...
def takeNodesOut(G, numNodesToRemove):
    ...
    Gdict = nx.betweenness_centrality(G, G.number_of_nodes())

    GdictTuple = Gdict.items()

    GdictTupleSorted = sorted(GdictTuple, key = itemgetter(1), reverse = True)

    GdictToRemove = GdictTupleSorted[:numNodesToRemove]

    nodesToRemove = []

    for (i, j) in GdictToRemove:
...
output_file_name = "resultBABC_N_%d_k_%d_SEED_%d.csv"%(N,k,SEED)
