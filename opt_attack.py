...
def dBalls_attack_adapt(G,radius, num_nodes_to_remove):
...
def opt_attack(G, radius, percent):

	N = G.numberOfNodes()

	num_nodes_to_remove = N * percent

	(GC_List_db, SGC_List, num_comp_List, avg_comp_size_List, counter_list,size_dball,size_ball,degree_list_mainNode,degree_list_removedNode,original_degree_main_node,original_degree_removed_node, original_xi_values) = dBalls_attack_adapt(G, radius, num_nodes_to_remove)

	newN = G.numberOfNodes()
