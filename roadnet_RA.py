...
def RA_attack(G_copy,num_nodes_to_remove):
	...
	return (GC_List, SGC_List, num_comp_List, avg_comp_List)
...
if road_net_type == "bj":
	...
	graph_type = "BEIJINGROAD"

	G_nx = nx.read_gpickle("BeijingU.gpickle")

	G = nk.nxadapter.nx2nk(G_nx)


elif road_net_type == "be":

	graph_type = "BELGRADEROAD"

	G_nx = nx.read_gpickle("BelgradeDriveU.gpickle")

	G = nk.nxadapter.nx2nk(G_nx)


elif road_net_type == "ny":

	graph_type = "NYROAD"

	G_nx = nx.read_gpickle("NYU.gpickle")
...
for i in range(10):
	...
	(avg_list_GC, avg_list_SGC, avg_list_numComp, avg_comp_list)  = RA_attack(G,int(N * 0.9))
	...
	GC_name = get_name_ER(init_name_GC_RAN, N, k, SEED,radius)

	SGC_name = get_name_ER(init_name_SGC_RAN, N, k, SEED,radius)

	numComp_name = get_name_ER(init_name_numComp_RAN, N, k, SEED,radius)

	avgSize_name = get_name_ER(init_name_avgSize_RAN, N, k, SEED,radius)



	with open(GC_name,'wb') as handle:
		pickle.dump(avg_list_GC, handle, protocol=pickle.HIGHEST_PROTOCOL)

	with open(SGC_name,'wb') as handle:
		pickle.dump(avg_list_SGC, handle, protocol=pickle.HIGHEST_PROTOCOL)

	with open(numComp_name,'wb') as handle:
		pickle.dump(avg_list_numComp, handle, protocol=pickle.HIGHEST_PROTOCOL)

	with open(avgSize_name,'wb') as handle:
